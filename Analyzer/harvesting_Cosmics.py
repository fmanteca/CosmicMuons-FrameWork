...
def makeVarPlot(hfile, dtree, hname, tag, collection, names, color=r.kRed, texts=[], ylog=True):
    ...
    h_muon.GetXaxis().SetLabelSize(0)
    ...
    h_diff.GetYaxis().SetLabelSize(0.075);
    h_diff.GetYaxis().SetNdivisions(6);
    h_diff.GetYaxis().SetTitleOffset(0.6);
    h_diff.GetXaxis().SetLabelSize(0.075);
    h_diff.GetYaxis().SetTitleSize(0.075);
    h_diff.GetXaxis().SetTitleSize(0.10);
    h_diff.GetXaxis().SetLabelOffset(0.02);
...
def make2DPlot(hfile, dtree, tag, hname, zlog=False, cut=True):
    ...
    c.Print(EOSPATH+'Plots/{0}/{1}/{2}.png'.format(tag,dtree.short_name,c.GetName()))
    c.Print(EOSPATH+'Plots/{0}/{1}/{2}.pdf'.format(tag,dtree.short_name,c.GetName()))
...
def make2DEfficiencyPlot(hfile, dtree, hname, tag, collection, zlog=False):
    ...
    c = r.TCanvas(hname+'_'+collection,hname+'_'+collection,ww=610,wh=600)
    c.SetFillStyle(4000)
    c.cd()
    if zlog: c.SetLogy(1)
    else: c.SetLogy(0)
...
if __name__ == '__main__':
    ...
    trees_originalFilter.append(DTree(['Cosmics_2022B_MiniAOD_ReReco', 'Cosmics_2022D_MiniAOD_ReReco', 'Cosmics_2022E_MiniAOD_ReReco'],              
                                       'Cosmics 2022 MiniAOD (ReReco BDE)', 
                                       [dat['Cosmics_2022G']['MiniAOD-Ntuples'], dat['Cosmics_2022F']['MiniAOD-Ntuples']],
                                       gTag, 
                                       short_name = 'Cosmics_2022BDE',
                                       isData = True))
    ...
    for dtree in trees_originalFilter + trees_nsegmentsFilter:
        ...
        #texts.append("NoBPTX3BX trigger, no ID cuts")
        #------------------------------------------------------------------------------#
